...
const panes = [
	{
		...
		render: ({ brand, loadingAdd, planModalOpen , setPlanModalOpen  , handleAddToVisited , isOpenLightBox , setIsOpenLightbox , images , onMovePrevRequest , onMoveNextRequest , onCloseRequest , photoIndex , handleOpenModal, user , pdfModalOpen , setPdfModalOpen }) => (
			<Tab.Pane attached={false} className="brand-tab-content">
				<div className="brand-tab-infos">
					...
					<div className="phone-number">
						...
						<a href={`tel:${brand.data.phone}`}><span>Tél. {brand.data.phone}</span></a>
					</div>
				</div>
							...
							<Grid.Column width={16}>
...

								{
									...
									<div style = {{ display : 'flex', gap : 10 }}>
										...
										{		
										!!restaurants?.find( current => brand.data.nom?.trim()?.toLocaleLowerCase()?.includes(current?.name?.trim()?.toLocaleLowerCase())) &&
											<>
												<Button
								...
								<Modal open = {pdfModalOpen} setOpen = { isOpen => setPdfModalOpen(isOpen) }>
									...
									<Slider autoplay = {false} pagination={true} id="brand-details">
										{restaurants?.find( current => brand.data.nom?.trim()?.toLocaleLowerCase()?.includes(current?.name?.trim()?.toLocaleLowerCase()))?.images?.map( image => (
											<Image style = {{ width : '100%' , objectFit : 'cover' }} key = {image} src = {image} alt = {image}  />
										) )}
									</Slider>			
								</Modal>
...
const OfferDetails = () => {
	...
	const handleOpenModal = () => !!user ? dispatch(openAddedNotification(true)) : dispatch(openAuthModal(true));
	...
	useEffect(() => {
		if(marque){
			const imgs = restaurants?.find( current => marque.data.nom?.trim()?.toLocaleLowerCase()?.includes(current?.name?.trim()?.toLocaleLowerCase()))?.images
			if(imgs){
				setImages(images)
			}
		}	
	...
	return loading ? (
		...
		<div className="brand-details">
			<>
				<Modal onUnmount = {handleCloseModal} open={open} setOpen={(value) => dispatch(openAddedNotification(value))}>
					<p> Ajouter {marque.data.nom} à la liste de visite ? </p>
					<Button loading = {loadingAdd} onClick={handleAddToVisited}>Confirmer </Button>
				</Modal>

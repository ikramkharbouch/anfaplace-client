...
const index = ({ menuOpen }) => {
	...
	const variants = {
		start: { opacity: 1, transition: { duration: 0.4, delay: 0.6 } },
		reverse: { opacity: 0, transition: { duration: 0.4 } }
	};
	...
	return (
		<motion.div className="coupons-section-container" ref={couponContainer} style={{ display: 'none' }} initial={{ y: 10, opacity: 0 }} variants={variants} animate={menuOpen ? 'start' : 'reverse'} onAnimationStart={handleStart} onAnimationComplete={handleComplete}>
			<div className='coupons-section-container'>


				<h3 className='app-small-title'> Coupons disponibles </h3>
				{
					coupons.length && (<div className="coupons-list">
						{
							coupons.map((coupon) => (
				...
				{!coupons.length && (<h3 className="not-available"> Vous n{`&apos;`}avez pas de coupons disponible </h3>)}
			</div>
		</motion.div>
